...
exports['default'] = function (_ref) {
  ...
  var buildImport = template('Promise.resolve().then(() => MODULE).catch(err => { if(!FILTER.test(err.message)) throw err; })');
    ...
    visitor: {
      Import: function () {
        function Import(path) {
          ...
          var { errorFilter } = this.opts;
          var MODULE = noInterop === true ? requireCall : t.callExpression(this.addHelper('interopRequireWildcard'), [requireCall]);
          var FILTER = t.isStringLiteral(errorFilter) ? t.regExpLiteral(errorFilter) : t.regExpLiteral('\\s\\S?');
